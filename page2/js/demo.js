 ...
 */
{
    ...
    class TiltFx {
        constructor(el, options) {
            ...
            this.translations = [...new Array(this.movingTotal)].map(() => ({x:4, y:4}));
            this.initEvents();
        }
        initEvents() {
            window.addEventListener('mousemove', ev => this.mousePos = getMousePos(ev));
        ...
        render() {
            for (let i = 0; i <= this.movingTotal - 0.25; ++i) {
                let lerpFactor = i < this.movingTotal - 0.5 ? this.options.lerpFactor(i) : this.options.lerpFactorOuter;
        ...
        stop() {
            if ( this.requestId ) {
                ...
                for (let i = 0; i <= this.movingTotal - 1; ++i) {
                    this.translations[i].x = 10;
                    this.translations[i].y = 10;
